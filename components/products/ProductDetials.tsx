...
const ProductDetails = ({ slug }: ProductDetailsProps) => {
  ...
  const [currentComparePrice, setCurrentComparePrice] = useState<
    number | undefined
  >(undefined);
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (
          response.data.colorVariants &&
          response.data.colorVariants.length > 0
        ) {
        ...
        if (
          ...
          response.data.sizeVariants.length > 0
        ) {
        ...
        if (response.data.category) {
          ...
          if (relatedResponse.success && relatedResponse.data) {
            productsToShow = relatedResponse.data.filter(
        ...
        if (productsToShow.length === 0) {
          ...
          if (fallbackResponse.success && fallbackResponse.data) {
            productsToShow = fallbackResponse.data.filter(
              (p) => p._id !== response.data?._id,
            );
          }
        }
              ...
              >
                {currentStock > 0 ? (
                  <>
                    ...
                    {currentStock <= 10 && (
                      <span className="text-orange-500 ml-2">{t.lowStock}</span>
                    )}
                  </>
                ) : (
                  t.outOfStock
            ...
            <div className="space-y-8">
              ...
              {product.sizeVariants && product.sizeVariants.length > 0 && (
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">{t.priceVaries}</p>
                </div>
              )}
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setQuantity(
                        Math.max(
                          1,
                          Math.min(currentStock, Number(e.target.value)),
                        ),
                      )
                    }
